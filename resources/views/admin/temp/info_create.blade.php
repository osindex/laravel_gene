    ...
    <div class="col-md-12">
        <div class="block">
            ...
            <div class="block-content block-content-narrow">
                {!! Form::open(['route' => "admin.{$table_en}.store"]) !!}
                    @if (isset($catagory))
                    ...
                    <div class="form-group">
                        {!! Form::label($col[1], $col[0].($col[2]>0?'*':'')) !!}
                    @if (isset($col[6]) && $col[6])
                        <div class="row">
                        @component('admin.partials.file',['animated'=>'fx-img-rotate-l','title'=>'图片','desc'=>'请选择需要上传的图片','defaultImg'=>'/assets/img/various/ecom_product2.png','fileName'=>'img','img'=>old($col[1])])
                        @endcomponent
                        </div>
                    @elseif(isset($col[5]) && $col[5])
                        {!! Form::text($col[1], old($col[1]), ['class' => 'form-control js-datepicker-'.$col[1],'data-date-format'=>"yyyy-mm-dd"]) !!}
                        @push('htmlstart')
                    ...
                    @elseif(isset($col[4]) && $col[4])
                        @component('admin.partials.editor',['label'=>$col[0],'content'=>old($col[1]),'required'=>'true','name'=>$col[1]])
                        @endcomponent
                    @elseif($col[3])
                        {!! Form::textarea($col[1], old($col[1]), ['class' => 'form-control','rows'=>'2']) !!}
                    @else
                        {!! Form::text($col[1], null, ['class' => 'form-control']) !!}
                    @endif
                    </div>
                    @endforeach
                    <div class="form-group">
